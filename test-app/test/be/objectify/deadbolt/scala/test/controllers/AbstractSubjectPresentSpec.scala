...
abstract class AbstractSubjectPresentSpec extends AbstractControllerSpec {
  ...
  "The application" should {
    "allow access if a subject is present" in new WithServer(app = testApp, port = 3333) {
      await(ws(implicitApp).url(s"http://localhost:3333/$pathSegment/subject/mustBePresent").withHttpHeaders(("x-deadbolt-test-user", "greet")).get()).status must equalTo(OK)
    }
