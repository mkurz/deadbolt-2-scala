...
import be.objectify.deadbolt.scala.test.controllers.{CompositionBased, AbstractControllerSpec}
  ...
  "The application" should {
    "deny access if" >> {
      "a subject is present but does not meet the pattern constraint" in new WithServer(app = testApp, port = 3333) {
        await(WS.url(s"http://localhost:3333/$pathSegment/composite/subjectDoesNotHavePermission")
      ...
      "the required role is held but the dynamic check fails" in new WithServer(app = testApp, port = 3333) {
        await(WS.url(s"http://localhost:3333/$pathSegment/composite/roleButNotDynamic")
      ...
      "the dynamic check passes but the required role is not help" in new WithServer(app = testApp, port = 3333) {
        await(WS.url(s"http://localhost:3333/$pathSegment/composite/noRoleButPassesDynamic")
    ...
    "allow access if" >> {
      ...
      "the required role is held and the dynamic check passes" in new WithServer(app = testApp, port = 3333) {
        await(WS.url(s"http://localhost:3333/$pathSegment/composite/hasRoleAndPassesDynamic")
              .withHeaders(("x-deadbolt-test-user", "trippel"))
              .get()).status must equalTo(OK)
      }
    }
