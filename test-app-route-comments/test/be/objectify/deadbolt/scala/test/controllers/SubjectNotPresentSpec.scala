...
class SubjectNotPresentSpec extends AbstractControllerSpec {
  ...
  "The application" should {
    "allow access if a subject is present" in new WithServer(app = testApp, port = 3333) {
      await(ws(implicitApp).url(s"http://localhost:3333/subject/mustNotBePresent").addHttpHeaders(("x-deadbolt-test-user", "greet")).get()).status must equalTo(UNAUTHORIZED)
    }
