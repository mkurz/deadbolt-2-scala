...
import be.objectify.deadbolt.scala.views.html.di.subjectNotPresentOr
import be.objectify.deadbolt.scala.views.html.di.subjectTest.subjectNotPresentOrContent
...
class SubjectNotPresentOrTest extends AbstractViewTest {
  ...
  def constraint(handler: DeadboltHandler) = new subjectNotPresentOrContent(new subjectNotPresentOr(viewSupport(), handlerCache(handler)))
}
