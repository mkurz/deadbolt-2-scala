...
import be.objectify.deadbolt.scala.views.html.di.subjectPresentOr
import be.objectify.deadbolt.scala.views.html.di.subjectTest.subjectPresentOrContent
...
class SubjectPresentOrTest extends AbstractViewTest {
  ...
  def constraint(handler: DeadboltHandler) = new subjectPresentOrContent(new subjectPresentOr(viewSupport(), handlerCache(handler)))
}
